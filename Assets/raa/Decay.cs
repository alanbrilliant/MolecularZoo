...
public class Decay : MonoBehaviour {
	...
	void Die()
	{
		...
		for(int i = 0;i < chance.Length; i++)
		{
			...
			if(chose < 0)
			{
				//get the molecule name by splitting the xxx.json file at the "." to get xxx (where xxx is the molecule name).
				string moleculeName = toSpawn[i].Substring(toSpawn[i].IndexOf("."));

				//These lines of code just spawn the molecule
				MoleculeCreator moleculeCreator = GameObject.FindObjectOfType<MoleculeCreator>();
				DataManager dataManager = GameObject.FindObjectOfType<DataManager>();
				MoleculeData moleculeData = dataManager.loadMolecule(toSpawn[i], moleculeName);
				GameObject spawned = moleculeCreator.instantiateMolecule(moleculeData, transform.position);
