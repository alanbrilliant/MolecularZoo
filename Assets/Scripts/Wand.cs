...
public class Wand : MonoBehaviour {
	...
	void Update () {
        ...
        if (controllerState == (int)arsenal.pistol) {
            GameObject currentActiveTool = null;
            foreach (GameObject gun in gunChildObjects)
            {
                if (gun.name == "Pistol")
            ...
            if (controller.GetHairTriggerDown()) {
				...
				GameObject shot = Instantiate (bullet, currentActiveTool.transform.position + currentActiveTool.transform.forward * .2f, currentActiveTool.transform.rotation);
